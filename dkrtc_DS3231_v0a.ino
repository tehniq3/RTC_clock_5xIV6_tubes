...
void setup() {
  watch.begin();  // Инициируем работу с модулем -> Initiating work with the module
  ...
  if (EEPROM.read(ADDR_INIT) != INIT_KEY) {  // проверка на первый запуск -> First-run test
  ...
  */
  watch.gettime();
  year = watch.year;  // текущее время перенос в переменные коррекции времени -> Current time is converted to variables time corrections
  month = watch.month;
  day = watch.day;
  hour = watch.Hours;
...
//svet_state = 0;  // just for test, leds off -> 
//delay(1000);
...
void loop() {
  ...
  if (millis() - timer >= PERIOD) 
  {
    ...
    Serial.println(watch.gettime("d-m-Y, H:i:s, D"));  // Выводим время -> Displaying the time
    do 
    {
      timer += PERIOD;
  ...
  {   // Сохранение настроек в ЭСППЗУ при флаге w3==1 -> Saving settings to EEPROM when the w3 flag is set to 1
    ...
    EEPROM.put(ADDR_SVET, svet_state);
    w3 = 0;
  }
